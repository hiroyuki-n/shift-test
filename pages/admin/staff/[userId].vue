          ...
          <dd>
            ...
            <div v-else class="space-y-1.5">
              <select
                ...
                class="w-full rounded-lg border border-slate-300 px-2 py-1.5 text-sm focus:border-brand focus:outline-none"
              >
              ...
              <div class="flex gap-1">
                <button
                  class="rounded-lg bg-brand px-2.5 py-1 text-xs font-semibold text-white hover:bg-brand-dark disabled:opacity-50"
                  :disabled="saving"
                  @click="saveEmploymentType"
                >{{ saving ? '保存中…' : '保存' }}</button>
                <button
                  class="rounded-lg border border-slate-300 px-2.5 py-1 text-xs text-slate-500 hover:bg-slate-50"
                  @click="cancelEdit"
                >取消</button>
              </div>
            </div>
